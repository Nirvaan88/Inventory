        ...
        <table id="tbl-iw-preview" style="width:100%;border-collapse:collapse;min-width:1000px;font-size:12px">
          <thead style="background:var(--bg-dark);position:sticky;top:0;z-index:2">
            <tr>
              ...
              ${isAdmin() ? '' : '<th style="padding:8px;text-align:center">Rate</th>'}
              ${isAdmin() ? '' : '<th style="padding:8px;text-align:right">Total Amt</th>'}
...
function _iwRenderItemPanel() {
  ...
  panel.innerHTML = `
    <div style="display:grid;grid-template-columns:${isAdmin() ? '2fr 1fr 1fr' : '2fr 1fr 1fr 1fr 1.2fr'};gap:10px;margin-bottom:10px;align-items:start">
      ...
      ${isAdmin() ? '' : `<div ${_IW_FIELD}><label ${_IW_LABEL}>Rate/Qty</label>
...
function _iwRenderPreview() {
  ...
  tbody.innerHTML = _iwLines.map((l, i) => {
    ...
    if (_iwEditId !== null) {
      ...
      ${isAdmin() ? '' : `<td style="padding:6px 8px;text-align:center">&#8377;${l.Rate}</td>`}
...
async function _iwShowRecords() {
    ...
    <div class="modal" style="max-width:980px;animation:slideUp 0.2s ease">
      <div class="modal-header">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          ${isSuperAdmin() ? `<button class="btn btn-sm" id="iw-rec-asp-btn" style="font-size:12.5px;background:#fef9c3;color:#92400e;border:1px solid #fde68a;border-radius:6px;padding:5px 13px;cursor:pointer;font-weight:600">
          ...
          <table style="width:100%;border-collapse:collapse;min-width:780px;font-size:13px" id="tbl-iw-records">
            <thead style="background:var(--bg-dark);position:sticky;top:0;z-index:2">
              <tr>
                ...
                ${isSuperAdmin() ? '<th style="padding:10px 12px;text-align:center;min-width:60px">ASP</th>' : ''}
              </tr>
            </thead>
  ...
  // Wire the Approval Sheet button (Super Admin only)
  const _aspBtn = document.getElementById('iw-rec-asp-btn');
  if (_aspBtn) _aspBtn.onclick = () => { ov.remove(); _iwShowApprovalSheet(); };
