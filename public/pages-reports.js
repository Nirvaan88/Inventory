...
function _chalRender() {
  ...
  const totalAmount = isAdmin() ? 0 : rows.reduce((s, r) => s + (Number(r.Amount) || 0), 0);
  ...
  const productRows = rows.map(r => `
    <tr style="background:#fff">
      ...
      <td ${_CE} style="padding:5px 8px;border:1px solid #ccc;text-align:right;color:#000;font-weight:bold;outline:none;cursor:text">${isAdmin() ? 0 : Number(r.Amount || 0).toFixed(0)}</td>
    </tr>`).join('');
            ...
            <table id="vis-rpt-table" style="width:100%;border-collapse:collapse;font-size:11.5px">
              <thead id="vis-rpt-head">
                <tr style="background:#f0f0f0;border-top:2px solid #333;border-bottom:1px solid #333">
                  ...
                  ${isAdmin() ? '' : '<th style="padding:5px 8px;text-align:right;border:1px solid #ccc">Sell Price</th>'}
                </tr>
              </thead>
...
function _visRender() {
  ...
  tbody.innerHTML = pageRows.length ? pageRows.map((r, i) => {
    ...
    return `<tr>
      ...
      ${isAdmin() ? '' : `<td style="${base}padding:4px 8px;text-align:right;border:1px solid #e0e0e0">${Number(r.SellPrice || 0).toFixed(2)}</td>`}
    </tr>`;
    ...
    <table>
      <thead>
        <tr>
          ...
          <th style="width:55px">UOM</th>${isAdmin() ? '' : '<th style="width:72px">Sell Price</th>'}
        </tr>
      </thead>
      <tbody>${rows}</tbody>
    </table>
...
registerPage('report-stock', async () => {
  ...
  <div class="card">
    <div class="table-wrapper"><table id="tbl-stock-report">
      <thead><tr><th>Sr No</th><th>Item Name</th><th>Category</th><th>Division</th><th>UOM</th><th>Stock</th><th>Reorder Level</th>${isAdmin() ? '' : '<th>Sell Price</th>'}<th>Status</th></tr></thead>
...
window._pageBinders['report-stock'] = async () => {
  ...
  const runReport = async () => {
    ...
    tbody.innerHTML = data.length ? data.map((d, i) => `<tr>
      ...
      ${isAdmin() ? '' : `<td>&#8377;${fmtNum(d.SellPrice)}</td>`}
