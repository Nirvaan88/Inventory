...
window._pageBinders['item-vendor-map'] = async () => {
  ...
  try { [_ivmDivisions, _ivmItems, _ivmVendors] = await Promise.all([
    api('/api/divisions?active=1'), api('/api/items'), api('/api/vendors?activeOnly=1')
  ]); } catch (_) {}
...
window._pageBinders['items'] = async () => {
  ...
  // Deduplicate by name — same category assigned to multiple divisions should appear once
  const seenCatNames = new Set();
  _itemCategories.forEach(c => {
    const name = (c.CategoryName || '').trim();
    if (!name || seenCatNames.has(name.toLowerCase())) return;
    seenCatNames.add(name.toLowerCase());
    const o = document.createElement('option');
    o.value = name;           // use NAME as value (not ID)
    o.textContent = name;
...
function applyItemFilters() {
  ...
  const catName = ($('#item-cat-filter')?.value || '').toLowerCase(); // now matched by name
  $$('#tbl-item-body tr:not(.empty-row)').forEach(tr => {
    const matchText = !q       || tr.textContent.toLowerCase().includes(q);
    const matchDiv  = !divId   || tr.dataset.divId === divId;
    // match all rows whose category name matches (covers all divisions)
    const matchCat  = !catName || (tr.dataset.catName || '').toLowerCase() === catName;
...
async function loadItems() {
  ...
  tbody.innerHTML = data.map(d => {
    ...
    return `
    <tr data-id="${d.itemid}" data-div-id="${d.DivisionId || ''}" data-cat-id="${d.CategoryId || ''}" data-cat-name="${(d.CategoryName || '').toLowerCase()}" class="item-row" style="${_rowStyle}">
      <td style="text-align:center">
