...
// -------- BULK UPLOAD HELPERS --------
window._downloadTemplate = function(headers, filename) {
  if (typeof XLSX === 'undefined') return showToast('SheetJS library not loaded', 'error');
...
window._buildBulkUploadHTML = function() {
  return `
    <div id="bulk-upload-section" style="display:none; padding: 10px 0;">
      <div style="margin-bottom:20px; padding:15px; background:var(--bg-dark); border:1px dashed var(--border); border-radius:8px; text-align:center">
        <p style="margin-bottom:10px; color:var(--text-secondary)">1. Download the template and fill in your data.</p>
        <button class="btn btn-secondary btn-sm" id="btn-download-tpl"><i class="fas fa-download"></i> Download Template</button>
      </div>
      <div style="margin-bottom:20px; padding:15px; background:var(--bg-dark); border:1px solid var(--border); border-radius:8px;">
        <p style="margin-bottom:10px; color:var(--text-secondary)">2. Upload the filled Excel file.</p>
        <input type="file" id="bulk-file-input" accept=".xlsx, .xls" style="width:100%; color:var(--text-primary)" />
      </div>
      <div id="bulk-preview-container" style="display:none">
        <h4 style="margin-bottom:10px; font-size:13px; color:var(--text-primary)">Preview (First 5 Rows)</h4>
        <div style="overflow-x:auto; margin-bottom:15px">
          <table style="width:100%; font-size:12px; border-collapse:collapse" id="bulk-preview-table">
            <thead></thead>
            <tbody></tbody>
          </table>
        </div>
...
window._handleBulkUploadLogic = function(overlay, endpointUrl, onSuccess, templateHeaders, templateFilename) {
  const fileInput = overlay.querySelector('#bulk-file-input');
  const btnDownload = overlay.querySelector('#btn-download-tpl');
  const previewContainer = overlay.querySelector('#bulk-preview-container');
  const previewTableHead = overlay.querySelector('#bulk-preview-table thead');
  const previewTableBody = overlay.querySelector('#bulk-preview-table tbody');
  
  let parsedData = [];

  btnDownload.onclick = () => window._downloadTemplate(templateHeaders, templateFilename);
  ...
  fileInput.onchange = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        parsedData = XLSX.utils.sheet_to_json(workbook.Sheets[firstSheet], { defval: "" });
        ...
        previewTableHead.innerHTML = '<tr>' + headers.map(h => `<th style="padding:4px 8px; border:1px solid var(--border); background:var(--bg-card)">${h}</th>`).join('') + '</tr>';
        
        const rowsHtml = parsedData.slice(0, 5).map(row => {
          return '<tr>' + headers.map(h => `<td style="padding:4px 8px; border:1px solid var(--border)">${row[h] || ''}</td>`).join('') + '</tr>';
        }).join('');
  ...
  return async () => {
    if (parsedData.length === 0) return showToast('Please select a valid Excel file first', 'error');
    
    const btnSave = overlay.querySelector('.modal-footer .btn-primary');
    const origHtml = btnSave.innerHTML;
    btnSave.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Uploading...';
    btnSave.disabled = true;

    try {
      const result = await api(endpointUrl, { method: 'POST', body: { rows: parsedData } });
      if (result.success) {
        const skipped = parsedData.length - (result.inserted || 0);
        let msg = `Bulk upload successful!\nInserted ${result.inserted || 0} rows.`;
        if (skipped > 0) {
          msg += `\nSkipped ${skipped} rows.`;
          if (result.skippedDetails && result.skippedDetails.length > 0) {
            msg += `\nReasons:\n` + result.skippedDetails.map(d => `- Row ${d.row}: ${d.reason}`).join('\n');
          } else {
            msg += ` (missing data or unrecognized references).`;
          }
        }
        alert(msg);
        overlay.remove();
...
window._addTabSwitcher = function(overlay) {
  const body = overlay.querySelector('.modal-body');
  
  const tabsHtml = `
    <div class="modal-tabs" style="display:flex; border-bottom:1px solid var(--border); margin-bottom:15px">
      <div class="modal-tab active" data-tab="single" style="padding:10px 20px; cursor:pointer; border-bottom:2px solid var(--accent); color:var(--accent); font-weight:600">Single Entry</div>
      <div class="modal-tab" data-tab="bulk" style="padding:10px 20px; cursor:pointer; color:var(--text-secondary); font-weight:500">Bulk Upload</div>
    </div>
  `;
  
  body.insertAdjacentHTML('afterbegin', tabsHtml);
  
  const singleSection = document.createElement('div');
  singleSection.id = 'single-entry-section';
  
  // Move all existing body children (except the newly added tabs) into singleSection
  const childrenToMove = Array.from(body.children).slice(1);
  childrenToMove.forEach(c => singleSection.appendChild(c));
  
  body.appendChild(singleSection);
  body.insertAdjacentHTML('beforeend', window._buildBulkUploadHTML());
  
  const tabs = overlay.querySelectorAll('.modal-tab');
  const btnSave = overlay.querySelector('.modal-footer .btn-primary');
  const origSaveHtml = btnSave.innerHTML;
  ...
  tabs.forEach(t => {
    t.onclick = () => {
      ...
      if (tabName === 'single') {
        overlay.querySelector('#single-entry-section').style.display = 'block';
        overlay.querySelector('#bulk-upload-section').style.display = 'none';
        btnSave.innerHTML = origSaveHtml;
      } else {
        overlay.querySelector('#single-entry-section').style.display = 'none';
        overlay.querySelector('#bulk-upload-section').style.display = 'block';
        btnSave.innerHTML = '<i class="fas fa-upload"></i> Upload Bulk';
      }
    };
  });
...
async function showAddCatModal() {
  ...
  const getCurrentMode = window._addTabSwitcher(overlay);
  const doBulkUpload = window._handleBulkUploadLogic(
    overlay, 
    '/api/categories/bulk-upload', 
    async () => await loadCategories(),
    ['Category Name', 'Division'],
    'category_bulk_template.xlsx'
  );

  const nameInp = $('#new-cat-name', overlay);
  nameInp.focus();
  nameInp.onkeydown = e => { if (e.key === 'Enter' && getCurrentMode() === 'single') doAdd(); };
  overlay.onclick = e => { if (e.target === overlay) overlay.remove(); };
  overlay.querySelector('.btn-close-modal').onclick = () => overlay.remove();
  overlay.querySelector('#btn-cat-add-save').onclick = () => {
    ...
    <div class="table-wrapper">
      <table id="tbl-ivm-main" style="min-width:1100px">
        ...
        <tbody id="tbl-ivm-body">
          <tr class="empty-row"><td colspan="9"><div class="spinner" style="margin:0 auto"></div></td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
async function loadIVM() {
  ...
  if (!tbody) return;
  tbody.innerHTML = `<tr class="empty-row"><td colspan="9"><div class="spinner" style="margin:0 auto"></div></td></tr>`;
  ...
  if (!data.length) {
    tbody.innerHTML = `<tr class="empty-row"><td colspan="9">No mappings found.</td></tr>`;
    return;
  }

  tbody.innerHTML = data.map(d => `
    <tr data-id="${d.ID}" data-div-id="${d.DivisionID || ''}" class="ivm-row">
      <td style="text-align:center">
      ...
      <td>${d.GST != null ? parseFloat(d.GST).toFixed(1) + '%' : '&#8212;'}</td>
...
function ivmModalBody(rec = {}) {
  return `
    ...
    <div class="form-row cols-2" style="gap:12px;margin-bottom:14px">
      ...
      <div class="form-field">
        <label>GST (%)</label>
        <input type="number" id="ivm-modal-gst" value="${rec.GST ?? ''}" min="0" step="0.1"
          style="background:var(--bg-dark);border:1px solid var(--border);border-radius:6px;
                 padding:9px 12px;color:var(--text-primary);width:100%;font-size:14px"
          placeholder="0.0"/>
      </div>
...
function showEditIVMInline(rec) {
  ...
  overlay.innerHTML = `
    <div class="modal" style="max-width:560px;animation:slideUp 0.2s ease">
      ...
      <div class="modal-body">${ivmModalBody(rec)}</div>
      <div class="modal-footer">
  ...
  document.getElementById('btn-ivm-inline-save').onclick = async () => {
    ...
    try {
      ...
      overlay.remove(); showToast('Mapping updated!', 'success'); await loadIVM();
    } catch (e) { showToast(e.message, 'error'); }
  };
}
...
async function showAddIVMModal() {
  ...
  const doBulkUpload = window._handleBulkUploadLogic(
    overlay, 
    '/api/item-vendor-mapping/bulk-upload', 
    async () => await loadIVM(),
    ['Division', 'Item Name', 'Vendor Name', 'Price Rs', 'GST (%)', 'Remark'],
    'item_vendor_mapping_bulk_template.xlsx'
  );

  overlay.onclick = e => { if (e.target === overlay) overlay.remove(); };
  overlay.querySelector('.btn-close-modal').onclick = () => overlay.remove();
  overlay.querySelector('#btn-ivm-add-save').onclick = async () => {
    if (getCurrentMode() === 'bulk') {
      doBulkUpload();
    } else {
      const b = collectIVMForm(overlay);
      if (!b.DivisionID) return showToast('Select a Division', 'error');
...
window.deleteIVM = async (id) => {
  if (!await confirm(`Delete Mapping ID ${id}? This cannot be undone.`)) return;
  try {
...
async function loadItems() {
  ...
  tbody.innerHTML = data.map(d => `
    <tr data-id="${d.itemid}" data-div-id="${d.DivisionId || ''}" data-cat-id="${d.CategoryId || ''}" class="item-row">
      <td style="text-align:center">
        <input type="checkbox" class="item-row-chk" data-id="${d.itemid}"
          style="width:16px;height:16px;cursor:pointer;accent-color:var(--accent)">
      </td>
      <td style="color:var(--text-secondary);font-size:13px">${d.itemid}</td>
      <td>
        <span style="background:var(--accent-soft);color:var(--accent);border-radius:20px;
      ...
      <td style="font-weight:600;color:var(--accent)">
        ${d.SellPrice != null ? '&#8377;' + parseFloat(d.SellPrice).toFixed(2) : '&#8212;'}
      </td>
  ...
  return `
    ...
    <div class="form-row cols-3" style="gap:12px;margin-bottom:14px">
      <div class="form-field">
        ...
        <input type="number" id="item-modal-price" value="${rec.SellPrice ?? ''}" min="0" step="0.01"
          style="background:var(--bg-dark);border:1px solid var(--border);border-radius:6px;
                 padding:9px 12px;color:var(--text-primary);width:100%;font-size:14px"
          placeholder="0.00"/>
      </div>
...
function collectItemForm(overlay) {
  return {
    ...
    SellPrice:    overlay.querySelector('#item-modal-price').value,
...
function showEditItemInline(rec) {
  ...
  overlay.innerHTML = `
    <div class="modal" style="max-width:600px;animation:slideUp 0.2s ease">
      ...
      <div class="modal-body">${itemModalBody(rec)}</div>
      <div class="modal-footer">
  ...
  document.getElementById('btn-item-inline-save').onclick = async () => {
    ...
    try {
      ...
      overlay.remove(); showToast('Item updated!', 'success'); await loadItems();
    } catch (e) { showToast(e.message, 'error'); }
  };
...
async function showAddItemModal() {
  ...
  const doBulkUpload = window._handleBulkUploadLogic(
    ...
    ['Division', 'Category', 'Item Name', 'UOM', 'Sell Price', 'Reorder Level', 'Reorder Qty', 'Priority (H/M/L)'],
    'item_master_bulk_template.xlsx'
  );
