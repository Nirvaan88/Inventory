...
try {
  sessionStore = new FileStore({
    ...
    reapInterval: 3600,          // only clean up stale sessions every hour
...
// =================== ACTIVITY LOG ===================
// Auto-create ActivityLog table on startup
(async () => {
  try {
    await query(`
    ...
    `);
    console.log('[ActivityLog] Table ensured.');
  } catch (e) {
    console.warn('[ActivityLog] Table creation warning:', e.message);
  }
})();

// logActivity — fire-and-forget, never blocks a response
async function logActivity(req, action, module, description) {
  try {
    ...
    await query(
      `INSERT INTO ActivityLog (UserName, RoleName, Action, Module, Description, IPAddress)
       VALUES (@u, @r, @a, @m, @d, @ip)`,
      {
        u:  u?.loginId  || 'system',
        r:  u?.roleName || '',
  ...
  } catch (_) { /* silent — never break a request for logging */ }
}

// Alias: insertLog(req, module, action, description) — same as logActivity with swapped arg order
async function insertLog(req, module, action, description) {
  return logActivity(req, action, module, description);
}
...
app.post('/api/login', async (req, res) => {
  ...
  // Hardcoded Super Admin bypass (admin/admin)
  if (loginId === 'admin' && password === 'admin') {
    const bypassUser = { loginId: 'admin', name: 'Super Admin', roleName: 'Super Admin', isAdmin: true };
    req.session.user = bypassUser;
    return req.session.save(err => {
      if (err) return res.status(500).json({ error: 'Session save failed' });
      logActivity(req, 'LOGIN', 'Authentication', `User 'admin' (Super Admin) logged in`);
      res.json({ success: true, user: bypassUser });
    });
  }

  try {
    // Query UserMaster joined with RoleMaster for role name
    const result = await query(
      `SELECT u.UserID, u.UserName, u.Name, u.RoleID, u.Status,
              r.Role AS RoleName
       FROM dbo.UserMaster u WITH (NOLOCK)
       LEFT JOIN dbo.RoleMaster r WITH (NOLOCK) ON u.RoleID = r.ID
       WHERE u.UserName = @lid
         AND u.Password = @pwd
         AND (u.Status = 1 OR u.Status = '1' OR u.Status = 'Y' OR u.Status = 'Active')`,
    ...
    const roleName = (u.RoleName || '').trim();
    const sessionUser = {
      loginId:  u.UserName,
      name:     u.Name || u.UserName,
      roleId:   u.RoleID,
      roleName: roleName,
      isAdmin:  (['super admin','superadmin','admin'].includes(roleName.toLowerCase()))
    };
    ...
    req.session.save(err => {
      ...
      logActivity(req, 'LOGIN', 'Authentication', `User '${sessionUser.loginId}' (${sessionUser.roleName}) logged in`);
...
app.post('/api/logout', (req, res) => {
  const u = req.session?.user;
  if (u) logActivity(req, 'LOGOUT', 'Authentication', `User '${u.loginId}' (${u.roleName}) logged out`);
  req.session.destroy();
...
app.post('/api/divisions', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'CREATE', 'Divisions', `Created Division: '${DivisionName}'`);
...
app.put('/api/divisions/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'UPDATE', 'Divisions', `Updated Division: '${DivisionName}' (ID: ${req.params.id})`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/divisions/:id', requireAuth, async (req, res) => {
  try {
    ...
    logActivity(req, 'DELETE', 'Divisions', `Deleted Division ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/divisions/bulk-delete', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'DELETE', 'Divisions', `Bulk deleted ${ids.length} Division(s): IDs [${ids.join(', ')}]`);
...
app.post('/api/departments', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'CREATE', 'Departments', `Created Department: '${DepName}'`);
...
app.put('/api/departments/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'UPDATE', 'Departments', `Updated Department: '${DepName}' (ID: ${req.params.id})`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/departments/:id', requireAuth, async (req, res) => {
  try {
    ...
    logActivity(req, 'DELETE', 'Departments', `Deleted Department ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/departments/bulk-delete', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'DELETE', 'Departments', `Bulk deleted ${ids.length} Department(s)`);
    res.json({ success: true, deleted: ids.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/categories', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'CREATE', 'Categories', `Created Category: '${CategoryName}'`);
...
app.delete('/api/categories/:id', requireAuth, async (req, res) => {
  try {
    ...
    logActivity(req, 'DELETE', 'Categories', `Deleted Category ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/categories/bulk-delete', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'DELETE', 'Categories', `Bulk deleted ${ids.length} Categor(ies)`);
    res.json({ success: true, deleted: ids.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    const r = await query(`
      SELECT i.[itemid], i.[ItemName], i.[CategoryId], i.[DivisionId],
             i.[UOM], i.[Stock], i.[ReorderLevel], i.[ReorderQty],
             i.[priority], i.[ItemCode], i.[FFlag],
             i.[Status], i.[AddedBy], i.[AddedDate], i.[ModifyBy], i.[ModifyDate],
             i.[ApprovalStatus], i.[LastRequestedBy], i.[AdminNotified],
             c.[CategoryName],
             d.[DivisionName]
      FROM [Item] i
...
app.post('/api/items', requireAuth, async (req, res) => {
  const { ItemName, CategoryId, DivisionId, ReorderLevel, ReorderQty, Stock, UOM, priority } = req.body;
  const user = req.session.user?.loginId || 'admin';
  const roleName = (req.session.user?.roleName || '').toLowerCase();
  const isAdminRole = roleName.includes('admin') && !roleName.includes('super');
  try {
    const r = await query(
      `INSERT INTO [Item] ([ItemName],[CategoryId],[DivisionId],[SellPrice],[ReorderLevel],[ReorderQty],[Stock],[UOM],[priority],[Status],[ApprovalStatus],[LastRequestedBy],[AdminNotified],[AddedBy],[AddedDate])
       OUTPUT INSERTED.[itemid]
       VALUES (@name,@catId,@divId,0,@reorderLevel,@reorderQty,@stock,@uom,@priority,'Y',@approvalStatus,@reqBy,0,@user,GETDATE())`,
      {
        ...
        approvalStatus: isAdminRole ? 'PENDING' : null,
        reqBy: isAdminRole ? user : null
      }
    );
    logActivity(req, 'CREATE', 'Item Master', `Created Item: '${ItemName}' (Status: ${isAdminRole ? 'PENDING' : 'Active'})`);
    if (isAdminRole) await insertLog(req, 'Item Master', 'Approval Request', `Approval request sent for new Item: '${ItemName}' (ID: ${r.recordset[0].itemid})`);
    res.json({ success: true, itemid: r.recordset[0].itemid, pending: isAdminRole });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    await query(
      `UPDATE [Item] SET [ItemName]=@name,[CategoryId]=@catId,[DivisionId]=@divId,
       ...
       [ApprovalStatus]=@approvalStatus,[LastRequestedBy]=@reqBy,[AdminNotified]=@notified
       WHERE [itemid]=@id`,
      {
        name: ItemName, catId: parseInt(CategoryId), divId: parseInt(DivisionId),
        reorderLevel: parseFloat(ReorderLevel) || 0,
        ...
        approvalStatus: isAdminRole ? 'PENDING' : null,
        reqBy: isAdminRole ? user : null,
        notified: isAdminRole ? 0 : null
      }
    );
    logActivity(req, 'UPDATE', 'Item Master', `Updated Item ID: ${req.params.id} ('${ItemName}')`);
    if (isAdminRole) await insertLog(req, 'Item Master', 'Approval Request', `Approval request sent after edit on Item ID: ${req.params.id} ('${ItemName}')`);
    res.json({ success: true, pending: isAdminRole });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
// Super Admin approves a pending item
app.put('/api/items/:id/approve', requireAuth, async (req, res) => {
  const roleName = (req.session.user?.roleName || '').toLowerCase().replace(/\s+/g,'');
  if (roleName !== 'superadmin') return res.status(403).json({ error: 'Super Admin only' });
  const { ItemName, CategoryId, DivisionId, ReorderLevel, ReorderQty, Stock, UOM, priority } = req.body;
  const user = req.session.user?.loginId || 'admin';
  try {
    await query(
      `UPDATE [Item] SET [ItemName]=@name,[CategoryId]=@catId,[DivisionId]=@divId,
       [ReorderLevel]=@reorderLevel,[ReorderQty]=@reorderQty,
       [Stock]=@stock,[UOM]=@uom,[priority]=@priority,[ModifyBy]=@user,[ModifyDate]=GETDATE(),
       [ApprovalStatus]=NULL,[AdminNotified]=0
       WHERE [itemid]=@id`,
      {
        name: ItemName, catId: parseInt(CategoryId), divId: parseInt(DivisionId),
        reorderLevel: parseFloat(ReorderLevel) || 0,
    ...
    logActivity(req, 'APPROVE', 'Item Master', `Approved Item ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/items/:id', requireAuth, async (req, res) => {
  try {
    ...
    logActivity(req, 'DELETE', 'Item Master', `Deleted Item ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/items/bulk-delete', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'DELETE', 'Item Master', `Bulk deleted ${ids.length} Item(s)`);
    res.json({ success: true, deleted: ids.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/vendors', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'CREATE', 'Vendor Master', `Created Vendor: '${Name}' (Company: ${CompanyName || 'N/A'})`);
...
app.put('/api/vendors/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'UPDATE', 'Vendor Master', `Updated Vendor ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/vendors/:id', requireAuth, async (req, res) => {
  try {
    ...
    logActivity(req, 'DELETE', 'Vendor Master', `Deleted Vendor ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/vendors/bulk-delete', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'DELETE', 'Vendor Master', `Bulk deleted ${ids.length} Vendor(s)`);
    res.json({ success: true, deleted: ids.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    let q = `
      SELECT ivm.[ItemId], i.[ItemName], i.[CategoryId], c.[CategoryName],
             ISNULL(ivm.[Total], ivm.[PriceRs]) AS DefaultRate, ivm.[DivisionID]
      FROM [ItemVendorMapping] ivm
      INNER JOIN [Item]     i ON ivm.[ItemId]   = i.[Itemid]
      INNER JOIN [Category] c ON i.[CategoryId] = c.[CategoryId]
      WHERE ivm.[Status] = 'Y'
        AND (ivm.[ApprovalStatus] IS NULL OR ivm.[ApprovalStatus] != 'PENDING')
        AND i.[Status]   = 'Y'
        AND (i.[ApprovalStatus] IS NULL OR i.[ApprovalStatus] != 'PENDING')
        AND c.[Status]   = 'Y'`;
    const params = {};
...
app.post('/api/inward', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'CREATE', 'Purchase Inward', `Created Inward ID: ${inwardId} (DC: ${DCNumber || 'N/A'}, Vendor ID: ${VendorId || 'N/A'}, ${(items||[]).length} items)`);
...
app.put('/api/inward/:inwardId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'UPDATE', 'Purchase Inward', `Updated Inward ID: ${inwardId} (DC: ${DCNumber || 'N/A'})`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/inward/:inwardId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'DELETE', 'Purchase Inward', `Deleted Inward ID: ${inwardId}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/dealers', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'CREATE', 'Dealer Master', `Created Dealer: '${f.DealerCompanyName || f.ContactPersonName || 'N/A'}'`);
...
app.put('/api/dealers/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'UPDATE', 'Dealer Master', `Updated Dealer ID: ${req.params.id} ('${f.DealerCompanyName || 'N/A'}')`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/dealers/:id', requireAuth, async (req, res) => {
  try {
    ...
    logActivity(req, 'DELETE', 'Dealer Master', `Deleted Dealer ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/dealers/bulk-delete', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'DELETE', 'Dealer Master', `Bulk deleted ${ids.length} Dealer(s)`);
    res.json({ success: true, deleted: ids.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/couriers', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'CREATE', 'Courier Master', `Created Courier: '${f.Name}'`);
...
app.put('/api/couriers/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'UPDATE', 'Courier Master', `Updated Courier ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/couriers/:id', requireAuth, async (req, res) => {
  try {
    ...
    logActivity(req, 'DELETE', 'Courier Master', `Deleted Courier ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/couriers/bulk-delete', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'DELETE', 'Courier Master', `Bulk deleted ${ids.length} Courier(s)`);
    res.json({ success: true, deleted: ids.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/users', requireAuth, async (req, res) => {
  try {
    const r = await query(`
      SELECT U.[UserID], U.[EmployeeID], U.[RoleID], R.[Role] AS [RoleName], U.[UserName], U.[Password], U.[SecurityQtn], U.[Answer], U.[Name], U.[EmailId], U.[MobileNo], U.[Gender], U.[Address], U.[City], U.[State], U.[Pin], U.[Country], U.[MaritalStatus], U.[AddedBy], U.[AddedDate], U.[ModifyBy], U.[ModifyDate], U.[Status]
      FROM [UserMaster] U
      LEFT JOIN [RoleMaster] R ON U.[RoleID] = R.[ID]
      ORDER BY U.[UserID]`);
    res.json(r.recordset);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/users', requireAuth, async (req, res) => {
  const { EmployeeID, RoleID, UserName, Password, SecurityQtn, Answer, Name, EmailId, MobileNo, Gender, Address, City, State, Pin, Country, MaritalStatus, Status } = req.body;
  const user = req.session.user?.loginId || 'admin';
  try {
    const r = await query(
      `INSERT INTO [UserMaster]([EmployeeID], [RoleID], [UserName], [Password], [SecurityQtn], [Answer], [Name], [EmailId], [MobileNo], [Gender], [Address], [City], [State], [Pin], [Country], [MaritalStatus], [Status], [AddedBy], [AddedDate])
       OUTPUT INSERTED.[UserID]
       VALUES(@empId, @roleId, @uname, @pwd, @sqtn, @ans, @name, @email, @mob, @gen, @addr, @city, @st, @pin, @coun, @mar, @status, @user, GETDATE())`,
      { 
        empId: EmployeeID || '', roleId: RoleID || null, uname: UserName || '', pwd: Password || '', sqtn: SecurityQtn || '', ans: Answer || '', name: Name || '',
        email: EmailId || '', mob: MobileNo || '', gen: Gender || '', addr: Address || '', city: City || '', st: State || '', pin: Pin || '', coun: Country || '',
        mar: MaritalStatus || '', status: Status || 'Y', user
      });
    logActivity(req, 'CREATE', 'User Master', `Created User: '${UserName}'`);
...
app.put('/api/users/:id', requireAuth, async (req, res) => {
  ...
  const user = req.session.user?.loginId || 'admin';
  try {
    let sql = `UPDATE [UserMaster] SET [EmployeeID]=@empId, [RoleID]=@roleId, [UserName]=@uname,
               [SecurityQtn]=@sqtn, [Answer]=@ans, [Name]=@name, [EmailId]=@email, [MobileNo]=@mob, [Gender]=@gen,
               [Address]=@addr, [City]=@city, [State]=@st, [Pin]=@pin, [Country]=@coun, [MaritalStatus]=@mar, [Status]=@status,
               [ModifyBy]=@user, [ModifyDate]=GETDATE()`;
    const params = {
      empId: EmployeeID || '', roleId: RoleID || null, uname: UserName || '', sqtn: SecurityQtn || '', ans: Answer || '', name: Name || '',
      email: EmailId || '', mob: MobileNo || '', gen: Gender || '', addr: Address || '', city: City || '', st: State || '', pin: Pin || '', coun: Country || '',
      mar: MaritalStatus || '', status: Status || 'Y', user, id: parseInt(req.params.id)
    };
    if (Password) {
      sql += `, [Password]=@pwd`;
      params.pwd = Password;
    ...
    logActivity(req, 'UPDATE', 'User Master', `Updated User ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/users/:id', requireAuth, async (req, res) => {
  try {
    ...
    logActivity(req, 'DELETE', 'User Master', `Deleted User ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/users/bulk-delete', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'DELETE', 'User Master', `Bulk deleted ${ids.length} User(s)`);
    res.json({ success: true, deleted: ids.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/users/export-xlsx', requireAuth, async (req, res) => {
  const { ids } = req.body;
  try {
    const base = `SELECT [UserID], [EmployeeID], [RoleID], [UserName], [Name], [EmailId], [MobileNo], [Status] FROM [UserMaster]`;
    let r;
    if (ids && ids.length) {
      const ph = ids.map((_, i) => `@id${i}`).join(',');
      const params = {}; ids.forEach((id, i) => { params[`id${i}`] = parseInt(id); });
...
// Role Master â€” full CRUD + bulk ops
app.get('/api/roles', requireAuth, async (req, res) => {
  try {
    const r = await query(`
      SELECT R.[ID], R.[Role], R.[Status], R.[AddedBy], R.[AddedDate], R.[ModifyBy], R.[ModifyDate],
             (SELECT STRING_AGG(U.UserName, ', ') FROM [UserMaster] U WHERE U.RoleID = R.ID) AS MappedUsers
      FROM [RoleMaster] R ORDER BY R.[ID]`);
    res.json(r.recordset);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/roles', requireAuth, async (req, res) => {
  ...
  try {
    const r = await query(
      `INSERT INTO [RoleMaster]([Role],[Status],[AddedBy],[AddedDate])
       OUTPUT INSERTED.[ID]
       VALUES(@role,@status,@user,GETDATE())`,
      { role: Role || '', status: Status || 1, user });
    logActivity(req, 'CREATE', 'Role Master', `Created Role: '${Role}'`);
...
app.put('/api/roles/:id', requireAuth, async (req, res) => {
  ...
  try {
    await query(
      `UPDATE [RoleMaster] SET [Role]=@role,[Status]=@status,
       [ModifyBy]=@user,[ModifyDate]=GETDATE()
       WHERE [ID]=@id`,
      { role: Role || '', status: Status || 1, user, id: parseInt(req.params.id) });
    logActivity(req, 'UPDATE', 'Role Master', `Updated Role ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/roles/:id', requireAuth, async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const c = await query(`SELECT COUNT(*) AS c FROM [UserMaster] WHERE [RoleID]=@id`, { id });
    if (c.recordset[0].c > 0) return res.status(400).json({ error: 'Cannot delete Role as it is mapped to one or more users.' });
    await query('DELETE FROM [RoleMaster] WHERE [ID]=@id', { id });
    logActivity(req, 'DELETE', 'Role Master', `Deleted Role ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/roles/bulk-delete', requireAuth, async (req, res) => {
  const { ids } = req.body;
  if (!ids || !ids.length) return res.status(400).json({ error: 'No IDs provided' });
  try {
    const ph = ids.map((_, i) => `@id${i}`).join(',');
    const params = {}; ids.forEach((id, i) => { params[`id${i}`] = parseInt(id); });
    const c = await query(`SELECT COUNT(*) AS c FROM [UserMaster] WHERE [RoleID] IN (${ph})`, params);
    if (c.recordset[0].c > 0) return res.status(400).json({ error: 'Cannot delete one or more selected roles as they are mapped to users.' });
    await query(`DELETE FROM [RoleMaster] WHERE [ID] IN (${ph})`, params);
    logActivity(req, 'DELETE', 'Role Master', `Bulk deleted ${ids.length} Role(s)`);
    res.json({ success: true, deleted: ids.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/roles/export-xlsx', requireAuth, async (req, res) => {
  const { ids } = req.body;
  try {
    const base = `
      SELECT R.[ID], R.[Role], R.[Status], (SELECT STRING_AGG(U.UserName, ', ') FROM [UserMaster] U WHERE U.RoleID = R.ID) AS MappedUsers
      FROM [RoleMaster] R`;
    let r;
    if (ids && ids.length) {
      const ph = ids.map((_, i) => `@id${i}`).join(',');
      const params = {}; ids.forEach((id, i) => { params[`id${i}`] = parseInt(id); });
      r = await query(`${base} WHERE R.[ID] IN (${ph}) ORDER BY R.[ID]`, params);
    } else { r = await query(`${base} ORDER BY R.[ID]`); }
    const ws = XLSX.utils.json_to_sheet(r.recordset);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'RoleMaster');
    const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
    res.setHeader('Content-Disposition', 'attachment; filename="role_master.xlsx"');
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.send(buf);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    const r = await query(`
      SELECT ivm.[ID], ivm.[ItemId], ivm.[VendorId], ivm.[DivisionID],
             ivm.[Remark], ivm.[PriceRs], ivm.[GST], ivm.[Total], ivm.[Status],
             ivm.[AddedBy], ivm.[AddedDate], ivm.[ModifyBy], ivm.[ModifyDate],
             ivm.[ApprovalStatus], ivm.[LastRequestedBy], ivm.[AdminNotified],
             i.[ItemName],
             v.[Name] AS VendorName,
             d.[DivisionName]
      FROM [ItemVendorMapping] ivm
...
app.post('/api/item-vendor-mapping', requireAuth, async (req, res) => {
  ...
  const roleName = (req.session.user?.roleName || '').toLowerCase();
  const isAdminRole = roleName.includes('admin') && !roleName.includes('super');
  const price = parseFloat(PriceRs) || 0;
  const gst   = parseFloat(GST) || 0;
  const total = parseFloat((price + (price * gst / 100)).toFixed(2));
  try {
    const r = await query(
      `INSERT INTO [ItemVendorMapping] ([ItemId],[VendorId],[DivisionID],[PriceRs],[GST],[Total],[Remark],[Status],[ApprovalStatus],[LastRequestedBy],[AdminNotified],[AddedBy],[AddedDate])
       OUTPUT INSERTED.[ID]
       VALUES (@itemId,@vendorId,@divId,@price,@gst,@total,@remark,'Y',@approvalStatus,@reqBy,0,@user,GETDATE())`,
      {
        itemId: parseInt(ItemId), vendorId: parseInt(VendorId), divId: parseInt(DivisionID),
        price, gst, total, remark: Remark || '', user,
        approvalStatus: isAdminRole ? 'PENDING' : null,
        reqBy: isAdminRole ? user : null
      }
    );
    logActivity(req, 'CREATE', 'Item-Vendor Mapping', `Created IVM ID: ${r.recordset[0].ID}`);
    if (isAdminRole) await insertLog(req, 'Item-Vendor Mapping', 'Approval Request', `Approval request sent for new IVM ID: ${r.recordset[0].ID} (Item: ${ItemId}, Vendor: ${VendorId})`);
    res.json({ success: true, ID: r.recordset[0].ID, pending: isAdminRole });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    await query(
      `UPDATE [ItemVendorMapping] SET
         ...
         [PriceRs]=@price, [GST]=@gst, [Total]=@total, [Remark]=@remark,
         [ModifyBy]=@user, [ModifyDate]=GETDATE(),
         [ApprovalStatus]=@approvalStatus,[LastRequestedBy]=@reqBy,[AdminNotified]=@notified
       WHERE [ID]=@id`,
      {
        itemId: parseInt(ItemId), vendorId: parseInt(VendorId), divId: parseInt(DivisionID),
        price, gst, total, remark: Remark || '', user, id: parseInt(req.params.id),
        approvalStatus: isAdminRole ? 'PENDING' : null,
        reqBy: isAdminRole ? user : null,
        notified: isAdminRole ? 0 : null
      }
    );
    logActivity(req, 'UPDATE', 'Item-Vendor Mapping', `Updated IVM ID: ${req.params.id}`);
    if (isAdminRole) await insertLog(req, 'Item-Vendor Mapping', 'Approval Request', `Approval request sent after edit on IVM ID: ${req.params.id}`);
    res.json({ success: true, pending: isAdminRole });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.put('/api/item-vendor-mapping/:id/approve', requireAuth, async (req, res) => {
  const roleName = (req.session.user?.roleName || '').toLowerCase().replace(/\s+/g,'');
  if (roleName !== 'superadmin') return res.status(403).json({ error: 'Super Admin only' });
  const { ItemId, VendorId, DivisionID, PriceRs, GST, Remark } = req.body;
  const user = req.session.user?.loginId || 'admin';
  ...
  try {
    ...
    logActivity(req, 'APPROVE', 'Item-Vendor Mapping', `Approved IVM ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/item-vendor-mapping/:id', requireAuth, async (req, res) => {
  try {
    ...
    logActivity(req, 'DELETE', 'Item-Vendor Mapping', `Deleted IVM ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/kisna-region-states', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'CREATE', 'Kisna Region States', `Created Kisna Region State: '${RegionName}'`);
    res.json({ success: true, ID: r.recordset[0].ID });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.delete('/api/kisna-region-states/:id', requireAuth, async (req, res) => {
  try {
    ...
    logActivity(req, 'DELETE', 'Kisna Region States', `Deleted Kisna Region State ID: ${req.params.id}`);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/category-codes', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logActivity(req, 'CREATE', 'Category / Item Codes', `Created Category Code: '${Code}'`);
    res.json({ success: true, ID: r.recordset[0].ID });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    ...
    let q = `SELECT [Itemid],[ItemName],[Stock],[ReorderLevel],
             ...
             FROM [Item] WHERE [Status]='Y'
               AND ([ApprovalStatus] IS NULL OR [ApprovalStatus] != 'PENDING')`;
    const params = {};
    if (divisionId) { q += ' AND [DivisionID]=@did'; params.did = parseInt(divisionId); }
    q += ' ORDER BY [ItemName]';
    const r = await query(q, params);
...
app.post('/api/issues', requireAuth, async (req, res) => {
  ...
  try {
    ...
    for (const it of (items || [])) {
      ...
      // Look up the current IVM price for this item (stored at transaction time)
      let ivmRate = 0;
      if (it.ItemId) {
        try {
          const rateRes = await query(`
            SELECT TOP 1 ISNULL(ivm.[Total], ivm.[PriceRs]) AS Rate
            FROM [ItemVendorMapping] ivm
            WHERE ivm.[ItemId] = @itemId AND ivm.[Status] = 'Y'
              AND (ivm.[ApprovalStatus] IS NULL OR ivm.[ApprovalStatus] != 'PENDING')
            ORDER BY ivm.[ID] DESC`, { itemId: it.ItemId });
          ivmRate = parseFloat(rateRes.recordset[0]?.Rate) || 0;
          // Fallback: try any IVM including PENDING
          if (!ivmRate) {
            const rateRes2 = await query(`
              SELECT TOP 1 ISNULL(ivm.[Total], ivm.[PriceRs]) AS Rate
              FROM [ItemVendorMapping] ivm
              WHERE ivm.[ItemId] = @itemId AND ivm.[Status] = 'Y'
              ORDER BY ivm.[ID] DESC`, { itemId: it.ItemId });
            ivmRate = parseFloat(rateRes2.recordset[0]?.Rate) || 0;
          }
        } catch (_) { }
      }
      const ivmAmount = issueQty * ivmRate;

      try {
        await query(
          `INSERT INTO [IssueItem]([IssueId],[ItemId],[RequestQty],[IssueQty],
             [PendingQty],[ItemFlag],[Status],[AddedBy],[AddedDate],[RequestId],[Rate],[Amount])
           VALUES(@iid,@itmid,@rqty,@iqty,@pqty,@flag,'Y',@user,GETDATE(),@reqid,@rate,@amount)`,
          {
            iid: issueId, itmid: it.ItemId || null, rqty: reqQty, iqty: issueQty,
            pqty: pendQty, flag, user, reqid: requestId,
            rate: ivmRate, amount: ivmAmount
          });
      } catch (_) { }
      // Decrement stock
    ...
    logActivity(req, 'CREATE', 'Issue Items', `Created Issue ID: ${issueId} (${(items||[]).length} items)`);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/challan/detail', requireAuth, async (req, res) => {
  try {
    ...
    if (!isSuperAdmin) {
      // Admin users: run the SP but zero out Rate & Amount (price hidden)
      const r = await query(
        'EXEC [dbo].[sp_GetDetailDataChallan] @intIssueID',
        { intIssueID: parseInt(issueId) }
      );
      return res.json(r.recordset.map(row => ({ ...row, Rate: 0, Amount: 0 })));
    }

    // Super Admin: hybrid approach —
    //   1. Use the Rate/Amount stored in IssueItem at transaction time (if non-zero)
    //   2. If stored Rate = 0 (pre-migration rows or IVM added after issue), fall back to live IVM lookup
    const enriched = await query(`
      SELECT
        ROW_NUMBER() OVER (ORDER BY ii.[IssueItemId]) AS SrlNo,
        it.[ItemName] AS ProdDesc,
        ii.[IssueQty] AS Pcs,
        ii.[ItemId],
        -- Stored rate from transaction time
        ISNULL(ii.[Rate], 0) AS StoredRate,
        -- Live IVM fallback: approved first, then any, then SellPrice
        ISNULL(
          (SELECT TOP 1 ISNULL(ivm.[Total], ivm.[PriceRs])
           FROM [ItemVendorMapping] ivm
           WHERE ivm.[ItemId] = ii.[ItemId] AND ivm.[Status]='Y'
             AND (ivm.[ApprovalStatus] IS NULL OR ivm.[ApprovalStatus] != 'PENDING')
           ORDER BY ivm.[ID] DESC),
          ISNULL(
            (SELECT TOP 1 ISNULL(ivm.[Total], ivm.[PriceRs])
             FROM [ItemVendorMapping] ivm
             WHERE ivm.[ItemId] = ii.[ItemId] AND ivm.[Status]='Y'
             ORDER BY ivm.[ID] DESC),
            ISNULL(it.[SellPrice], 0)
          )
        ) AS LiveRate,
        ii.[IssueId] AS Issueid
      FROM [IssueItem] ii
      INNER JOIN [Item] it ON ii.[ItemId] = it.[ItemId]
      WHERE ii.[IssueId] = @iid
      ORDER BY ii.[IssueItemId]`,
      { iid: parseInt(issueId) }
    );

    // Pick best rate and compute Amount
    const result = enriched.recordset.map(row => {
      const rate   = (parseFloat(row.StoredRate) || 0) > 0
                     ? parseFloat(row.StoredRate)
                     : (parseFloat(row.LiveRate) || 0);
      const qty    = parseInt(row.Pcs) || 0;
    ...
    // Also update IssueItem with the resolved rate for future calls (async, non-blocking)
    result.forEach(row => {
      if (row.Rate > 0) {
        query(`UPDATE [IssueItem] SET [Rate]=@rate, [Amount]=@amt
               WHERE [IssueId]=@iid AND [Rate]=0
                 AND [ItemId]=(SELECT [ItemId] FROM [Item] WHERE [ItemName]=@name)`,
          { rate: row.Rate, amt: row.Amount, iid: parseInt(issueId), name: row.ProdDesc })
          .catch(() => {});
      }
    });
...
app.get('/api/activity-logs', requireAuth, async (req, res) => {
  ...
  if (roleName !== 'superadmin') return res.status(403).json({ error: 'Super Admin only' });
  try {
    const { module: mod, action, user, from, to, search, page = 1, limit = 50 } = req.query;
    const offset = (parseInt(page) - 1) * parseInt(limit);
    const params = {};
    const conditions = [];

    if (mod)    { conditions.push('Module = @mod');     params.mod    = mod; }
    if (action) { conditions.push('Action = @action');  params.action = action; }
    if (user)   { conditions.push('UserName LIKE @user'); params.user = `%${user}%`; }
    if (from)   { conditions.push('CAST(LoggedAt AS DATE) >= @from'); params.from = from; }
    if (to)     { conditions.push('CAST(LoggedAt AS DATE) <= @to');   params.to   = to; }
    if (search) { conditions.push('Description LIKE @search'); params.search = `%${search}%`; }

    const where = conditions.length ? 'WHERE ' + conditions.join(' AND ') : '';

    const countRes = await query(`SELECT COUNT(*) AS total FROM ActivityLog ${where}`, params);
    const total = countRes.recordset[0].total;

    params.lmt    = parseInt(limit);
    params.offset = offset;
    const rows = await query(
      `SELECT LogID, UserName, RoleName, Action, Module, Description, IPAddress,
              FORMAT(LoggedAt, 'dd-MMM-yyyy HH:mm:ss') AS LoggedAt
       FROM ActivityLog ${where}
       ORDER BY LoggedAt DESC
       OFFSET @offset ROWS FETCH NEXT @lmt ROWS ONLY`,
      params
    );

    // Distinct modules for filter dropdown
    const modulesRes = await query(`SELECT DISTINCT Module FROM ActivityLog ORDER BY Module`);
    ...
    res.json({
      ...
      modules: modulesRes.recordset.map(r => r.Module),
      logs: rows.recordset
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/alerts', requireAuth, async (req, res) => {
  const roleName = (req.session.user?.roleName || '').toLowerCase();
  const user = req.session.user?.loginId || '';
  try {
    if (roleName.replace(/\s+/g,'') === 'superadmin') {
      const [items, ivm] = await Promise.all([
        query(`SELECT i.[itemid], i.[ItemName], i.[LastRequestedBy], i.[ModifyDate] AS RequestedAt,
                       c.[CategoryName], d.[DivisionName]
                FROM [Item] i
                LEFT JOIN [Category] c ON i.[CategoryId]=c.[CategoryId]
                LEFT JOIN [Division] d ON i.[DivisionId]=d.[DivisionId]
                WHERE i.[ApprovalStatus]='PENDING'
                ORDER BY i.[ModifyDate] DESC`),
        query(`SELECT ivm.[ID], ivm.[LastRequestedBy], ivm.[ModifyDate] AS RequestedAt,
                       i.[ItemName], v.[Name] AS VendorName, d.[DivisionName],
                       ivm.[PriceRs], ivm.[GST], ivm.[Remark], ivm.[ItemId], ivm.[VendorId], ivm.[DivisionID]
                FROM [ItemVendorMapping] ivm
                LEFT JOIN [Item] i ON ivm.[ItemId]=i.[itemid]
                LEFT JOIN [Vendor] v ON ivm.[VendorId]=v.[VendorId]
                LEFT JOIN [Division] d ON ivm.[DivisionID]=d.[DivisionId]
                WHERE ivm.[ApprovalStatus]='PENDING'
                ORDER BY ivm.[ModifyDate] DESC`)
      ]);
      res.json({
        pendingItems: items.recordset,
        pendingIVM: ivm.recordset,
        total: items.recordset.length + ivm.recordset.length
      });
    } else {
      const [items, ivm] = await Promise.all([
        query(`SELECT i.[itemid], i.[ItemName], i.[ModifyDate] AS ApprovedAt
                FROM [Item] i
                WHERE i.[LastRequestedBy]=@user AND i.[ApprovalStatus] IS NULL AND i.[AdminNotified]=0`,
               { user }),
        query(`SELECT ivm.[ID], ivm.[ModifyDate] AS ApprovedAt, i.[ItemName], v.[Name] AS VendorName
                FROM [ItemVendorMapping] ivm
                LEFT JOIN [Item] i ON ivm.[ItemId]=i.[itemid]
                LEFT JOIN [Vendor] v ON ivm.[VendorId]=v.[VendorId]
                WHERE ivm.[LastRequestedBy]=@user AND ivm.[ApprovalStatus] IS NULL AND ivm.[AdminNotified]=0`,
               { user })
      ]);
      res.json({
        approvedItems: items.recordset,
        approvedIVM: ivm.recordset,
        total: items.recordset.length + ivm.recordset.length
      });
    }
  } catch (e) { res.status(500).json({ error: e.message }); }
...
app.post('/api/alerts/dismiss', requireAuth, async (req, res) => {
  const { itemIds = [], ivmIds = [] } = req.body;
  try {
    if (itemIds.length) {
      const pl = itemIds.map((_, i) => `@id${i}`).join(',');
      const params = {}; itemIds.forEach((id, i) => { params[`id${i}`] = parseInt(id); });
      await query(`UPDATE [Item] SET [AdminNotified]=1 WHERE [itemid] IN (${pl})`, params);
    }
    if (ivmIds.length) {
      const pl = ivmIds.map((_, i) => `@id${i}`).join(',');
      const params = {}; ivmIds.forEach((id, i) => { params[`id${i}`] = parseInt(id); });
      await query(`UPDATE [ItemVendorMapping] SET [AdminNotified]=1 WHERE [ID] IN (${pl})`, params);
    }
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/categories/bulk-upload', requireAuth, async (req, res) => {
  const { rows } = req.body;
  ...
  try {
    const divs = (await query('SELECT DivisionID, DivisionName FROM Division')).recordset;
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      const catName = (row['Category Name'] || '').toString().trim();
      const divName = (row['Division Name'] || '').toString().trim();
      if (!catName) { skipped++; errors.push(`Row ${i+1}: Category Name is required.`); continue; }
      
      let divId = null;
      if (divName) {
        const d = divs.find(x => (x.DivisionName || '').toLowerCase() === divName.toLowerCase());
        if (!d) { skipped++; errors.push(`Row ${i+1}: Division '${divName}' not found.`); continue; }
        divId = d.DivisionID;
      }
      
      const chk = await query('SELECT 1 FROM Category WHERE CategoryName = @name', { name: catName });
      if (chk.recordset.length > 0) { skipped++; errors.push(`Row ${i+1}: Category '${catName}' already exists.`); continue; }
      
      await query(`INSERT INTO Category (CategoryName, DivisionId, AddedBy, AddedDate) VALUES (@name, @divId, @user, GETDATE())`,
        { name: catName, divId, user });
      inserted++;
    }
    if (inserted > 0) await insertLog(req, 'Category Master', 'Bulk Upload', `Imported ${inserted} categories.`);
...
app.post('/api/items/bulk-upload', requireAuth, async (req, res) => {
  const { rows } = req.body;
  if (!rows || !rows.length) return res.status(400).json({ error: 'No data provided' });
  const user = req.session.user?.loginId || 'admin';
  const roleName = (req.session.user?.roleName || '').toLowerCase().replace(/\s+/g, '');
  ...
  try {
    const cats = (await query('SELECT CategoryId, CategoryName FROM Category')).recordset;
    const divs = (await query('SELECT DivisionID, DivisionName FROM Division')).recordset;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const catName      = (row['Category Name'] || '').toString().trim();
      const divName      = (row['Division Name'] || '').toString().trim();
      const uom          = (row['UOM']           || 'Pcs').toString().trim();
      const stock        = parseFloat(row['Stock'])        || 0;
      const reorderLevel = parseFloat(row['Reorder Level']) || 0;
      const reorderQty   = parseFloat(row['Reorder Qty'])   || 0;
      const itemCode     = (row['Item Code']     || '').toString().trim();
      
      if (!itemName) { skipped++; errors.push(`Row ${i+1}: Item Name is required.`); continue; }
      
      let catId = null, divId = null;
      if (catName) {
        const c = cats.find(x => (x.CategoryName || '').toLowerCase() === catName.toLowerCase());
        if (!c) { skipped++; errors.push(`Row ${i+1}: Category '${catName}' not found.`); continue; }
        catId = c.CategoryId;
      }
      if (divName) {
        const d = divs.find(x => (x.DivisionName || '').toLowerCase() === divName.toLowerCase());
      ...
      const chk = await query('SELECT 1 FROM Item WHERE ItemName = @name', { name: itemName });
      if (chk.recordset.length > 0) { skipped++; errors.push(`Row ${i+1}: Item '${itemName}' already exists.`); continue; }
      
      // Admin-inserted items go PENDING; super admin items are active immediately
      const approvalStatus = isAdminRole ? 'PENDING' : null;
      const adminNotified  = isAdminRole ? 0 : null;
      await query(
        `INSERT INTO [Item]
           ([ItemName],[CategoryId],[DivisionId],[UOM],[Stock],[ReorderLevel],[ReorderQty],[ItemCode],
            [AddedBy],[AddedDate],[ApprovalStatus],[LastRequestedBy],[AdminNotified])
         VALUES
           (@name,@catId,@divId,@uom,@stock,@reorderLevel,@reorderQty,@itemCode,
            @user,GETDATE(),@apst,@reqBy,@notified)`,
        { name: itemName, catId, divId, uom, stock, reorderLevel, reorderQty,
          itemCode: itemCode || null, user,
          apst: approvalStatus, reqBy: isAdminRole ? user : null, notified: adminNotified });
      if (isAdminRole) await insertLog(req, 'Item Master', 'Approval Request', `Approval request for bulk-uploaded item '${itemName}'`);
      inserted++;
    }
    if (inserted > 0) await insertLog(req, 'Item Master', 'Bulk Upload', `Imported ${inserted} items.`);
    res.json({ inserted, skipped, errors });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/api/vendors/bulk-upload', requireAuth, async (req, res) => {
  const { rows } = req.body;
  if (!rows || !rows.length) return res.status(400).json({ error: 'No data provided' });
  const user = req.session.user?.loginId || 'admin';
  let inserted = 0, skipped = 0, errors = [];
  ...
  try {
    const states = (await query('SELECT StateID, [State] FROM [State]')).recordset;
    const cities  = (await query('SELECT CityID, [City] FROM [City]')).recordset;
    
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      const name = (row['Vendor Name'] || '').toString().trim();
      const mob  = (row['Mobile'] || '').toString().trim();
      const stateName = (row['State'] || '').toString().trim();
      const cityName  = (row['City']  || '').toString().trim();
      
      if (!name) { skipped++; errors.push(`Row ${i+1}: Vendor Name is required.`); continue; }
      
      let stateId = null, cityId = null;
      if (stateName) {
        const s = states.find(x => (x.State || '').toLowerCase() === stateName.toLowerCase());
        if (!s) { skipped++; errors.push(`Row ${i+1}: State '${stateName}' not found.`); continue; }
        stateId = s.StateID;
      }
      if (cityName) {
        const c = cities.find(x => (x.City || '').toLowerCase() === cityName.toLowerCase());
        if (!c) { skipped++; errors.push(`Row ${i+1}: City '${cityName}' not found.`); continue; }
        cityId = c.CityID;
      }
      
      const chk = await query('SELECT 1 FROM Vendor WHERE Name = @name', { name });
      if (chk.recordset.length > 0) { skipped++; errors.push(`Row ${i+1}: Vendor '${name}' already exists.`); continue; }
      
      await query(
        `INSERT INTO [Vendor] ([Name],[Mob],[Addr1],[Addr2],[CityID],[StateID],[Pin],[CompanyName],[PAN],[AadharNo],[GSTNo],[BankName],[BankAccNo],[IFSCCode],[VendorEmail],[AddedBy],[AddedDate])
         VALUES (@name,@mob,@addr1,@addr2,@cityId,@stateId,@pin,@company,@pan,@aadhar,@gst,@bank,@bankAcc,@ifsc,@email,@user,GETDATE())`,
        {
          name, mob,
          addr1:   (row['Address 1']    || '').toString().trim(),
          addr2:   (row['Address 2']    || '').toString().trim(),
          cityId, stateId,
          pin:     (row['Pin']          || '').toString().trim(),
          company: (row['Company Name'] || '').toString().trim(),
          pan:     (row['PAN']          || '').toString().trim(),
          aadhar:  (row['Aadhar No']   || '').toString().trim(),
          gst:     (row['GST No']       || '').toString().trim(),
          bank:    (row['Bank Name']    || '').toString().trim(),
          bankAcc: (row['Bank Acc No']  || '').toString().trim(),
          ifsc:    (row['IFSC Code']    || '').toString().trim(),
          email:   (row['Vendor Email'] || '').toString().trim(),
          user
        });
      inserted++;
    }
    if (inserted > 0) await insertLog(req, 'Vendor Master', 'Bulk Upload', `Imported ${inserted} vendors.`);
    res.json({ inserted, skipped, errors });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/api/item-vendor-mapping/bulk-upload', requireAuth, async (req, res) => {
  const { rows } = req.body;
  if (!rows || !rows.length) return res.status(400).json({ error: 'No data provided' });
  const user = req.session.user?.loginId || 'admin';
  const roleName = (req.session.user?.roleName || '').toLowerCase().replace(/\s+/g, '');
  ...
  try {
    const divs    = (await query('SELECT DivisionID, DivisionName FROM Division')).recordset;
    // Load ALL items (including pending) so names always resolve
    const allItems  = (await query('SELECT ItemId, ItemName FROM Item WHERE Status IS NULL OR Status=\'Y\' OR Status=1')).recordset;
    const vendors  = (await query('SELECT VendorId, Name FROM Vendor')).recordset;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const price  = parseFloat(row['Price Rs']) || 0;
      const gst    = parseFloat(row['GST %'])    || 0;
      const total  = parseFloat((price + (price * gst / 100)).toFixed(2));
      const remark = (row['Remark'] || '').toString().trim();
      
      if (!itemName || !vendorName || !divName) {
        skipped++; errors.push(`Row ${i+1}: Division, Item, and Vendor names are required.`); continue;
      }
      ...
      const it = allItems.find(x => (x.ItemName || '').toLowerCase() === itemName.toLowerCase());
      if (!it) { skipped++; errors.push(`Row ${i+1}: Item '${itemName}' not found.`); continue; }
      const itemId = it.ItemId;
      
      const v = vendors.find(x => (x.Name || '').toLowerCase() === vendorName.toLowerCase());
      if (!v) { skipped++; errors.push(`Row ${i+1}: Vendor '${vendorName}' not found.`); continue; }
      const vendorId = v.VendorId;
      
      const chk = await query(
        'SELECT 1 FROM ItemVendorMapping WHERE ItemId=@itemId AND VendorId=@vendorId AND DivisionID=@divId',
        { itemId, vendorId, divId });
      if (chk.recordset.length > 0) { skipped++; errors.push(`Row ${i+1}: Mapping already exists for '${itemName}'.`); continue; }
      
      const approvalStatus = isAdminRole ? 'PENDING' : null;
      const adminNotified  = isAdminRole ? 0 : null;
      await query(
        `INSERT INTO [ItemVendorMapping]
           ([ItemId],[VendorId],[DivisionID],[PriceRs],[GST],[Total],[Remark],[AddedBy],[AddedDate],[ApprovalStatus],[LastRequestedBy],[AdminNotified])
         VALUES (@itemId,@vendorId,@divId,@price,@gst,@total,@remark,@user,GETDATE(),@apst,@reqBy,@notified)`,
        { itemId, vendorId, divId, price, gst, total, remark, user,
          apst: approvalStatus, reqBy: isAdminRole ? user : null, notified: adminNotified });
      if (isAdminRole) await insertLog(req, 'Item-Vendor Mapping', 'Approval Request', `Approval request for bulk-uploaded IVM: Item '${itemName}', Vendor '${vendorName}'`);
      inserted++;
    }
    if (inserted > 0) await insertLog(req, 'Item-Vendor Mapping', 'Bulk Upload', `Imported ${inserted} mappings.`);
    res.json({ inserted, skipped, errors });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
