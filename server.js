...
app.post('/api/categories/bulk-upload', requireAuth, async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const chk = await query('SELECT 1 FROM Category WHERE CategoryName = @p0', [catName]);
      if (chk.recordset.length > 0) { skipped++; errors.push(`Row ${i+1}: Category '${catName}' already exists.`); continue; }
      
      await query(`INSERT INTO Category (CategoryName, DivisionId, AddedBy, AddedDate) VALUES (@p0, @p1, @p2, GETDATE())`, [catName, divId, user]);
      inserted++;
    }
...
app.post('/api/items/bulk-upload', requireAuth, async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const chk = await query('SELECT 1 FROM Item WHERE ItemName = @p0', [itemName]);
      if (chk.recordset.length > 0) { skipped++; errors.push(`Row ${i+1}: Item '${itemName}' already exists.`); continue; }
      
      await query(`INSERT INTO [Item] ([ItemName],[CategoryId],[DivisionId],[Description],[Stock],[AddedBy],[AddedDate]) VALUES (@p0,@p1,@p2,@p3,@p4,@p5,GETDATE())`, [itemName, catId, divId, desc, stock, user]);
      inserted++;
    }
...
app.post('/api/vendors/bulk-upload', requireAuth, async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const chk = await query('SELECT 1 FROM Vendor WHERE Name = @p0', [name]);
      if (chk.recordset.length > 0) { skipped++; errors.push(`Row ${i+1}: Vendor '${name}' already exists.`); continue; }
      
      await query(`INSERT INTO [Vendor] ([Name],[Mob],[Addr1],[Addr2],[CityID],[StateID],[Pin],[CompanyName],[PAN],[AadharNo],[GSTNo],[BankName],[BankAccNo],[IFSCCode],[VendorEmail],[AddedBy],[AddedDate]) 
        VALUES (@p0,@p1,@p2,@p3,@p4,@p5,@p6,@p7,@p8,@p9,@p10,@p11,@p12,@p13,@p14,@p15,GETDATE())`, 
        [name, mob, (row['Address 1']||'').toString().trim(), (row['Address 2']||'').toString().trim(), cityId, stateId, (row['Pin']||'').toString().trim(), 
         (row['Company Name']||'').toString().trim(), (row['PAN']||'').toString().trim(), (row['Aadhar No']||'').toString().trim(), (row['GST No']||'').toString().trim(),
         (row['Bank Name']||'').toString().trim(), (row['Bank Acc No']||'').toString().trim(), (row['IFSC Code']||'').toString().trim(), (row['Vendor Email']||'').toString().trim(), user]);
      inserted++;
    }
...
app.post('/api/item-vendor-mapping/bulk-upload', requireAuth, async (req, res) => {
  ...
  try {
    const divs = (await query('SELECT DivisionID, DivisionName FROM Division')).recordset;
    const items = (await query('SELECT Itemid, ItemName FROM Item')).recordset;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!itemName || !vendorName || !divName) { skipped++; errors.push(`Row ${i+1}: Division, Item, and Vendor names are required.`); continue; }
      
      const d = divs.find(x => (x.DivisionName || '').toLowerCase() === divName.toLowerCase());
      if (!d) { skipped++; errors.push(`Row ${i+1}: Division '${divName}' not found.`); continue; }
      const divId = d.DivisionID;
      
      const it = items.find(x => (x.ItemName || '').toLowerCase() === itemName.toLowerCase());
      ...
      const chk = await query('SELECT 1 FROM ItemVendorMapping WHERE ItemId = @p0 AND VendorId = @p1 AND DivisionID = @p2', [itemId, vendorId, divId]);
      if (chk.recordset.length > 0) { skipped++; errors.push(`Row ${i+1}: Mapping already exists.`); continue; }
      
      await query(`INSERT INTO [ItemVendorMapping] ([ItemId],[VendorId],[DivisionID],[PriceRs],[GST],[Total],[Remark],[AddedBy],[AddedDate]) 
        VALUES (@p0,@p1,@p2,@p3,@p4,@p5,@p6,@p7,GETDATE())`, [itemId, vendorId, divId, price, gst, total, remark, user]);
      inserted++;
    }
