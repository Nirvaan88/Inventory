      ...
      await query(
        `INSERT INTO [Item]
           ([ItemName],[CategoryId],[DivisionId],[UOM],[Stock],[ReorderLevel],[ReorderQty],[ItemCode],
            [Status],[AddedBy],[AddedDate],[ApprovalStatus],[LastRequestedBy],[AdminNotified])
         VALUES
           (@name,@catId,@divId,@uom,@stock,@reorderLevel,@reorderQty,@itemCode,
            'Y',@user,GETDATE(),@apst,@reqBy,@notified)`,
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      await query(
        `INSERT INTO [ItemVendorMapping]
           ([ItemId],[VendorId],[DivisionID],[PriceRs],[GST],[Total],[Remark],
            [Status],[AddedBy],[AddedDate],[ApprovalStatus],[LastRequestedBy],[AdminNotified])
         VALUES (@itemId,@vendorId,@divId,@price,@gst,@total,@remark,
                 'Y',@user,GETDATE(),@apst,@reqBy,@notified)`,
