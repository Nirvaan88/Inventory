...
app.post('/api/login', async (req, res) => {
  ...
  // Admin bypass
  if (loginId === 'sadmin' && password === 'sad123') {
    req.session.user = { loginId: 'sadmin', isAdmin: true };
    ...
    return req.session.save(err => {
      ...
      res.json({ success: true, user: { loginId: 'sadmin', isAdmin: true } });
    });
  }

  try {
    const result = await query(
      `SELECT u.*, r.Role AS RoleName FROM UserMaster u WITH (NOLOCK) 
       LEFT JOIN RoleMaster r WITH (NOLOCK) ON u.RoleID = r.ID 
       WHERE u.UserName = @lid AND u.Password = @pwd AND (u.Status = 1 OR u.Status = '1' OR u.Status = 'Y' OR u.Status = 'Active')`,
    ...
    req.session.user = { 
      ...
      roleId: user.RoleID,
      roleName: roleName,
      isAdmin: (roleName.toLowerCase() === 'super admin' || roleName.toLowerCase() === 'admin')
    };
...
app.post('/api/categories/bulk-upload', requireAuth, async (req, res) => {
  ...
  const user = req.session.user?.loginId || 'admin';
  try {
    const divs = await query(`SELECT DivisionId, DivisionName FROM Division`);
    const divMap = {};
    divs.recordset.forEach(d => { 
      divMap[String(d.DivisionName || '').toLowerCase().trim()] = d.DivisionId; 
      divMap[String(d.DivisionId)] = d.DivisionId;
    });
    ...
    for (const row of rows) {
      rowIndex++;
      const catName = row['Category Name'] || row['CategoryName'];
      const divName = row['Division'] || row['DivisionName'];
      if (!catName || !divName) {
        skippedDetails.push({ row: rowIndex, reason: 'Missing Category Name or Division' });
        continue;
      }
      
      const divId = divMap[String(divName).toLowerCase().trim()];
      if (!divId) {
        skippedDetails.push({ row: rowIndex, reason: `Division "${divName}" not found` });
        continue;
      }
      ...
      try {
        await query(
          `INSERT INTO Category (CategoryName, DivisionId, Status, AddedBy, AddedDate)
           VALUES (@name, @divId, 'Y', @user, GETDATE())`,
          { name: String(catName).trim(), divId, user }
        );
        inserted++;
      } catch (err) {
        skippedDetails.push({ row: rowIndex, reason: err.message });
      }
    }
    res.json({ success: true, inserted, skippedDetails });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/items/bulk-upload', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const cats = await query(`SELECT CategoryId, CategoryName FROM Category`);
    const catMap = {};
    cats.recordset.forEach(c => { 
      catMap[String(c.CategoryName || '').toLowerCase().trim()] = c.CategoryId; 
      catMap[String(c.CategoryId)] = c.CategoryId;
    });
    ...
    for (const row of rows) {
      ...
      if (!divName || !catName || !itemName || !uom) {
        skippedDetails.push({ row: rowIndex, reason: 'Missing Division, Category, Item Name, or UOM' });
        continue;
      }

      const divId = divMap[String(divName).toLowerCase().trim()];
      const catId = catMap[String(catName).toLowerCase().trim()];
      if (!divId || !catId) {
        skippedDetails.push({ row: rowIndex, reason: 'Division or Category not found in database' });
        continue;
      }
      ...
      const priority = row['Priority (H/M/L)'] || 'M';

      try {
        await query(
          `INSERT INTO [Item] ([ItemName],[CategoryId],[DivisionId],[SellPrice],[ReorderLevel],[ReorderQty],[Stock],[UOM],[priority],[Status],[AddedBy],[AddedDate])
           VALUES (@name,@catId,@divId,@sellPrice,@reorderLevel,@reorderQty,0,@uom,@priority,'Y',@user,GETDATE())`,
          {
            name: String(itemName).trim(), catId, divId,
            sellPrice, reorderLevel, reorderQty,
            uom: String(uom).trim(), priority: String(priority).trim(), user
          }
        );
        inserted++;
      } catch (err) {
...
app.post('/api/vendors/bulk-upload', requireAuth, async (req, res) => {
  ...
  const user = req.session.user?.loginId || 'admin';
  try {
    const states = await query(`SELECT StateID, State FROM State`);
    const stateMap = {};
    states.recordset.forEach(s => { 
      stateMap[String(s.State || '').toLowerCase().trim()] = s.StateID; 
      stateMap[String(s.StateID)] = s.StateID;
    });

    const cities = await query(`SELECT CityID, City FROM City`);
    const cityMap = {};
    cities.recordset.forEach(c => { 
      cityMap[String(c.City || '').toLowerCase().trim()] = c.CityID; 
      cityMap[String(c.CityID)] = c.CityID;
    });
    ...
    for (const row of rows) {
      ...
      if (!name) {
        skippedDetails.push({ row: rowIndex, reason: 'Missing Vendor Name' });
        continue;
      }
      ...
      const stateId = stateName ? stateMap[String(stateName).toLowerCase().trim()] || null : null;
      const cityId = cityName ? cityMap[String(cityName).toLowerCase().trim()] || null : null;
      ...
      try {
        await query(
           ...
           VALUES (@name,@mob,@addr1,@addr2,@cityId,@stateId,@pin,@company,@gst,@pan,@aadhar,@bank,@bankAcc,@ifsc,@vemail,'Y',@user,GETDATE())`,
          {
            ...
            mob: String(row['Mobile'] || '').trim().substring(0, 10), 
            addr1: String(row['Address 1'] || '').trim().substring(0, 100), 
            addr2: String(row['Address 2'] || '').trim().substring(0, 100),
            cityId, stateId,
            pin: String(row['Pin'] || '').replace(/\.0$/, '').trim().substring(0, 6), 
            company: String(row['Company Name'] || '').trim().substring(0, 100), 
            gst: String(row['GST No'] || '').trim().substring(0, 50), 
            pan: String(row['PAN'] || '').trim().substring(0, 10),
            aadhar: String(row['Aadhar No'] || '').trim().replace(/[^0-9]/g, '').substring(0, 16), 
            bank: String(row['Bank Name'] || '').trim().substring(0, 100), 
            bankAcc: String(row['Bank Acc No'] || '').trim().substring(0, 50), 
            ifsc: String(row['IFSC Code'] || '').trim().substring(0, 50),
            vemail: String(row['Vendor Email'] || '').trim().substring(0, 255), 
            user
          }
        );
        inserted++;
...
app.get('/api/roles', requireAuth, async (req, res) => {
  try {
    const r = await query(`
      SELECT r.[ID], r.[Role], r.[AddedBy], r.[AddedDate], r.[ModifyBy], r.[ModifyDate], r.[Status],
...
app.delete('/api/roles/:id', requireAuth, async (req, res) => {
  try {
    await query(`DELETE FROM [RoleMaster] WHERE [ID]=@id`, { id: parseInt(req.params.id) });
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    const base = `
      SELECT r.[ID], r.[Role], r.[Status],
             (SELECT STRING_AGG(u.[UserName], ', ') FROM [UserMaster] u WHERE u.[RoleID] = r.[ID]) AS [Mapped Users]
      FROM [RoleMaster] r`;
    let rQuery;
    if (ids && ids.length) {
      const ph = ids.map((_, i) => `@id${i}`).join(',');
      const params = {}; ids.forEach((id, i) => { params[`id${i}`] = parseInt(id); });
      rQuery = await query(`${base} WHERE r.[ID] IN (${ph}) ORDER BY r.[ID]`, params);
    } else { 
      rQuery = await query(`${base} ORDER BY r.[ID]`); 
    }
    const ws = XLSX.utils.json_to_sheet(rQuery.recordset);
    const wb = XLSX.utils.book_new();
...
app.post('/api/item-vendor-mapping/bulk-upload', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const items = await query(`SELECT itemid, ItemName FROM Item`);
    const itemMap = {};
    items.recordset.forEach(i => { 
      itemMap[String(i.ItemName || '').toLowerCase().trim()] = i.itemid; 
      itemMap[String(i.itemid)] = i.itemid;
    });

    const vendors = await query(`SELECT vendorid, Name FROM Vendor`);
    const vendorMap = {};
    vendors.recordset.forEach(v => { 
      vendorMap[String(v.Name || '').toLowerCase().trim()] = v.vendorid; 
      vendorMap[String(v.vendorid)] = v.vendorid;
    });
    ...
    for (const row of rows) {
      ...
      if (!divName || !itemName || !vendorName) {
        skippedDetails.push({ row: rowIndex, reason: 'Missing Division, Item Name, or Vendor Name' });
        continue;
      }

      const divId = divMap[String(divName).toLowerCase().trim()];
      const itemId = itemMap[String(itemName).toLowerCase().trim()];
      const vendorId = vendorMap[String(vendorName).toLowerCase().trim()];

      if (!divId || !itemId || !vendorId) {
        skippedDetails.push({ row: rowIndex, reason: 'Division, Item, or Vendor not found in database' });
        continue;
      }

      const priceRs = parseFloat(row['Price Rs']) || 0;
      ...
      try {
        await query(
           ...
           VALUES (@itemId,@vendorId,@divId,@price,@gst,@remark,'Y',@user,GETDATE())`,
          {
            ...
            price: priceRs, gst,
            remark: String(remark).trim(), user
          }
        );
        inserted++;
      } catch (err) {
...
app.get('/api/users', requireAuth, async (req, res) => {
  try {
    const r = await query(`
      SELECT u.UserID, u.EmployeeID, u.RoleID, u.UserName, u.Password, u.SecurityQtn, u.Answer, 
             u.Name, u.EmailId, u.MobileNo, u.Gender, u.Address, u.City, u.State, u.pin, u.Country, 
             u.MaritalStatus, u.Status, r.Role AS RoleName
...
app.post('/api/users', requireAuth, async (req, res) => {
  ...
  if (!RoleID || !UserName || !Password || !SecurityQtn || !Answer || !Name || !EmailId || !MobileNo) {
    return res.status(400).json({ error: 'Missing mandatory fields' });
  }

  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(EmailId)) return res.status(400).json({ error: 'Invalid email address' });
  const mobileRegex = /^\d{10}$/;
  if (!mobileRegex.test(MobileNo)) return res.status(400).json({ error: 'Mobile number must be exactly 10 digits' });



  try {
    // Check duplicate username
    const dup = await query(`SELECT UserID FROM UserMaster WHERE UserName = @un`, { un: UserName });
    if (dup.recordset.length > 0) return res.status(400).json({ error: 'Username already exists' });
    ...
    await query(`
      ...
      VALUES (@emp, @role, @un, @pwd, @sq, @ans, @name, @email, @mob, @gen, @addr, @city, @st, @pin, @coun, @mar, @status, @added, GETDATE())
    `, {
      emp: EmployeeID || '',
      role: RoleID,
      un: UserName,
      ...
      mar: MaritalStatus || '',
      status: Status || 1,
      added: req.session.user?.name || 'admin'
    });
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.put('/api/users/:id', requireAuth, async (req, res) => {
  ...
  if (!RoleID || !UserName || !SecurityQtn || !Answer || !Name || !EmailId || !MobileNo) {
    return res.status(400).json({ error: 'Missing mandatory fields' });
  }

  try {
    // Duplicate check
    const dup = await query(`SELECT UserID FROM UserMaster WHERE UserName = @un AND UserID != @id`, { un: UserName, id });
    if (dup.recordset.length > 0) return res.status(400).json({ error: 'Username already taken' });

    let sql = `
      UPDATE UserMaster SET 
        EmployeeID=@emp, RoleID=@role, UserName=@un, SecurityQtn=@sq, Answer=@ans, 
        Name=@name, EmailId=@email, MobileNo=@mob, Gender=@gen, Address=@addr, 
        City=@city, State=@st, pin=@pin, Country=@coun, MaritalStatus=@mar, 
        Status=@status, ModifyBy=@mod, ModifyDate=GETDATE()
    `;
    const params = {
      id, emp: EmployeeID || '', role: RoleID, un: UserName, sq: SecurityQtn, ans: Answer,
      name: Name, email: EmailId, mob: MobileNo, gen: Gender || '', addr: Address || '', 
      city: City || '', st: State || '', pin: Pin || '', coun: Country || '', mar: MaritalStatus || '', 
      status: Status || 1, mod: req.session.user?.name || 'admin'
    };
...
app.post('/api/users/bulk-delete', requireAuth, async (req, res) => {
  ...
  if (!ids || !ids.length) return res.status(400).json({ error: 'No IDs provided' });
  try {
    const marks = ids.map((_, i) => `@id${i}`).join(',');
    const params = {};
    ids.forEach((id, i) => { params[`id${i}`] = id; });
    await query(`DELETE FROM UserMaster WHERE UserID IN (${marks})`, params);
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/api/users/export-xlsx', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const r = await query(`
      SELECT u.UserID, u.EmployeeID, u.UserName, u.Name, u.EmailId, u.MobileNo, r.Role AS RoleName, u.Status
      FROM UserMaster u WITH (NOLOCK)
      LEFT JOIN RoleMaster r WITH (NOLOCK) ON u.RoleID = r.ID
      WHERE u.UserID IN (${marks})
    `, params);
...
// Forgot Password flows (these do not use requireAuth since user isn't logged in)
app.post('/api/users/forgot-password/init', async (req, res) => {
  const { UserName } = req.body;
  try {
    const r = await query(`SELECT SecurityQtn FROM UserMaster WITH (NOLOCK) WHERE UserName = @un`, { un: UserName });
    if (r.recordset.length === 0) return res.status(404).json({ error: 'User not found' });
    res.json({ success: true, SecurityQtn: r.recordset[0].SecurityQtn });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

app.post('/api/users/forgot-password/verify', async (req, res) => {
  const { UserName, SecurityQtn, Answer } = req.body;
  try {
    const r = await query(`SELECT UserID FROM UserMaster WITH (NOLOCK) WHERE UserName = @un AND SecurityQtn = @sq AND Answer = @ans`, 
      { un: UserName, sq: SecurityQtn, ans: Answer });
    if (r.recordset.length === 0) return res.status(400).json({ error: 'Invalid answers' });
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/users/forgot-password/reset', async (req, res) => {
  ...
  try {
    await query(`UPDATE UserMaster SET Password = @pwd, ModifyDate=GETDATE() WHERE UserName = @un`, { pwd: NewPassword, un: UserName });
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
